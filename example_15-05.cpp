...
public:
    ...
    int size_check(cv::Mat &I) { //Check that image doesn't change size, return -1 if size doesn't match, else 0
        ...
        if ((row != I.rows) || (col != I.cols)) {
            cerr << "ERROR: Size changed! old[" << row << ", " << col << "], now [" << I.rows << ", " << I.cols << "]!"
                 << endl;
            ret = -1;
        }
        return ret;
...
void adjustThresholds(char *charstr, cv::Mat &Irgb, CbBackgroudDiff &bgd) {
    ...
    while ((key = cv::waitKey()) != 27 && key != 'Q' && key != 'q')  // Esc or Q or q to exit
    {
        ...
        cout << "y,u,v for channel; l for minMod, h for maxMod threshold; , for down, . for up; esq or q to quit;"
             << endl;
...
int main() {
    cv::namedWindow("example_15-05", cv::WINDOW_AUTOSIZE);
    cv::VideoCapture cap;
