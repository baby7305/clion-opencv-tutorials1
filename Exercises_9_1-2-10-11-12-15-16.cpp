...
int main() {
    ...
    */
    ...
    imshow("5 x 5 Gaussian filter", temp);
    //b
    GaussianBlur(singleChanel100, temp, cv::Size(9, 9), 0);
    imshow("9 x 9Gaussian filter", temp);
    //c
    GaussianBlur(singleChanel100, temp, cv::Size(5, 5), 0);
    ...
    */
    ...
    // create 9 lines
    for (int i = 0; i < 9; i++) {
        line(matWithLines, Point(arng.uniform(0, 512), arng.uniform(0, 521)),
             Point(arng.uniform(0, 512), arng.uniform(0, 521)), Scalar(255), 1);
    }
    //45 degree line
    ...
    imshow("a 3 x 3 Gaussian kernel", temp);
    //b
