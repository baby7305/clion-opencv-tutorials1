...
int main() {
    ...
    cv::FileStorage fs("C:\\opencv\\Learning-OpenCV-3_examples-master\\birdseye\\intrinsics.xml",
                       cv::FileStorage::READ);
    ...
    if (!fs.isOpened() || intrinsic.empty() || distortion.empty()) {
        cout << "Error: Couldn't load intrinsic parameters from "
             << "Learning-OpenCV-3_examples-master\\birdseye\\intrinsics.xml"
             << endl;
        return -1;
    }
    fs.release();

    cv::Mat gray_image, image, image0 = cv::imread(
            "C:\\opencv\\Learning-OpenCV-3_examples-master\\birdseye\\IMG_0215L.jpg", 1);
    if (image0.empty()) {
        cout << "Error: Couldn't load image "
             << "C:\\opencv\\Learning-OpenCV-3_examples-master\\birdseye\\IMG_0215L.jpg" << endl;
        return -1;
    }
    ...
    if (!found) {
        cout << "Couldn't acquire checkerboard on "
             << "C:\\opencv\\Learning-OpenCV-3_examples-master\\birdseye\\IMG_0215L.jpg" << ", only found "
