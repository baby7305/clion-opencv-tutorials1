...
void showForgroundInRed(const cv::Mat &img) {
    cv::Mat rawImage;
    cv::split(img, Igray);
    Igray[2] = cv::max(mask, Igray[2]);
    cv::merge(Igray, rawImage);
    cv::imshow("example_15-03", rawImage);
    cv::imshow("Segmentation", mask);
}

void adjustThresholds(cv::Mat &img) {
    int key = 1;
    while ((key = cv::waitKey()) != 27 && key != 'Q' && key != 'q')  // Esc or Q or q to exit
    {
        ...
        if (key == 'h') { high_thresh -= 0.2; }
        cout << "H or h, L or l, esq or q to quit;  high_thresh = " << high_thresh << ", " << "low_thresh = "
             << low_thresh << endl;
        setHighThreshold(high_thresh);
        setLowThreshold(low_thresh);
        backgroundDiff(img, mask);
        showForgroundInRed(img);
    }
}

int main() {
    cv::namedWindow("example_15-03", cv::WINDOW_AUTOSIZE);
    cv::VideoCapture cap;
    if (!cap.open("C:\\opencv\\Learning-OpenCV-3_examples-master//tree.avi")) {
    ...
    while (1) {
        ...
        cv::imshow("example_15-03", image);
        image_count++;
        if ((key = cv::waitKey(7)) == 27 || key == 'q' || key == 'Q' || image_count >= number_to_train_on)
    ...
    cout
            << "Done!  Hit any key to continue into single step. Hit 'a' or 'A' to adjust thresholds, esq, 'q' or 'Q' to quit\n"
            << endl;
    ...
    while ((key = cv::waitKey()) != 27 || key == 'q' || key == 'Q') { // esc, 'q' or 'Q' to exit
        ...
        //
        showForgroundInRed(image);
        if (key == 'a') {
            cout << "In adjust thresholds, 'H' or 'h' == high thresh up or down; 'L' or 'l' for low thresh up or down."
                 << endl;
            cout << " esq, 'q' or 'Q' to quit " << endl;
            adjustThresholds(image);
